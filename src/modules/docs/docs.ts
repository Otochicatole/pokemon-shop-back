...
export function buildOpenApi(): any {
  ...
  registry.registerPath({ method: 'get', path: '/api/v1/catalog/products', tags: ['Catalog'], responses: { 200: { description: 'Published products', content: { 'application/json': { schema: z.object({ data: z.array(productSchema), nextCursor: z.string().nullable() }) } } } } });
  registry.registerPath({ method: 'get', path: '/api/v1/checkout/options', tags: ['Checkout'], responses: { 200: { description: 'Available fulfillment and payment methods', content: { 'application/json': { schema: checkoutOptionsSchema } } } } });
  registry.registerPath({ method: 'post', path: '/api/v1/checkout/preview', tags: ['Checkout'], security: [{ userCookie: [] }], request: { body: { required: true, content: { 'application/json': { schema: orderInputSchema } } } }, responses: { 200: { description: 'Server-calculated quote', content: { 'application/json': { schema: z.object({ subtotal: moneySchema, shipping: moneySchema, total: moneySchema }) } } }, 409: { description: 'Product or stock changed', content: { 'application/json': { schema: problemSchema } } } } });
  registry.registerPath({ method: 'post', path: '/api/v1/orders', tags: ['Orders'], security: [{ userCookie: [] }], request: { headers: z.object({ 'idempotency-key': z.string() }), body: { required: true, content: { 'application/json': { schema: orderInputSchema } } } }, responses: { 201: { description: 'Order created' }, 409: { description: 'Conflict', content: { 'application/json': { schema: problemSchema } } } } });
