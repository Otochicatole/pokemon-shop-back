...
export function registerAdminCmsPaths(registry: OpenAPIRegistry, { problemSchema, moneySchema }: Options): void {
  ...
  const affiliateIdParams = z.object({ id: z.string().uuid() });
  const affiliateAdminProfile = z.object({ id: z.string().uuid(), userId: z.string().uuid(), publicName: z.string(), status: z.enum(['ACTIVE', 'SUSPENDED']), version: z.number().int() }).passthrough();
  const affiliateReview = z.object({ expectedVersion: z.number().int().min(1), decision: z.enum(['APPROVED', 'CHANGES_REQUESTED', 'REJECTED']), note: z.string().max(500).optional() });
  ...
  registry.registerPath({ method: 'get', path: '/api/v2/admin/affiliates', tags: ['Admin affiliates'], summary: 'List affiliate profiles and seller activity', security, responses: ok('Affiliate profiles', z.array(affiliateAdminProfile)) });
  registry.registerPath({ method: 'post', path: '/api/v2/admin/affiliates', tags: ['Admin affiliates'], summary: 'Enable an existing customer as an affiliate', security, request: { headers: csrf, body: json(z.object({ userId: z.string().uuid(), publicName: z.string().min(2).max(120), contactPhone: z.string().max(50).nullable().optional(), payoutAccount: z.string().max(500).nullable().optional() })) }, responses: created('Affiliate created', affiliateAdminProfile) });
  registry.registerPath({ method: 'patch', path: '/api/v2/admin/affiliates/{id}', tags: ['Admin affiliates'], summary: 'Suspend or reactivate an affiliate using optimistic concurrency', security, request: { params: affiliateIdParams, headers: csrf, body: json(z.object({ expectedVersion: z.number().int().min(1), publicName: z.string().max(120).optional(), status: z.enum(['ACTIVE', 'SUSPENDED']).optional() })) }, responses: ok('Affiliate updated', affiliateAdminProfile) });
  registry.registerPath({ method: 'get', path: '/api/v2/admin/affiliates/listings', tags: ['Admin affiliates'], summary: 'List affiliate publications awaiting review', security, responses: ok('Affiliate listings', z.array(z.unknown())) });
  registry.registerPath({ method: 'post', path: '/api/v2/admin/affiliates/listings/{id}/review', tags: ['Admin affiliates'], summary: 'Approve, reject or request changes on an affiliate listing', security, request: { params: affiliateIdParams, headers: csrf, body: json(affiliateReview) }, responses: ok('Affiliate listing reviewed', z.unknown()) });
  registry.registerPath({ method: 'get', path: '/api/v2/admin/affiliates/payouts', tags: ['Admin affiliates'], summary: 'List affiliate payout requests', security, responses: ok('Affiliate payout requests', z.array(z.unknown())) });
  registry.registerPath({ method: 'post', path: '/api/v2/admin/affiliates/payouts/{id}/process', tags: ['Admin affiliates'], summary: 'Record an external affiliate payout or rejection', security, request: { params: affiliateIdParams, headers: csrf, body: json(z.object({ expectedVersion: z.number().int().min(1), status: z.enum(['PAID', 'REJECTED']), externalReference: z.string().min(2).max(150), note: z.string().max(500).optional() })) }, responses: ok('Payout processed', z.unknown()) });
  registry.registerPath({ method: 'get', path: '/api/v2/admin/affiliates/settings', tags: ['Admin affiliates'], summary: 'Read the versioned global affiliate commission', security, responses: ok('Affiliate settings', z.unknown()) });
  registry.registerPath({ method: 'patch', path: '/api/v2/admin/affiliates/settings', tags: ['Admin affiliates'], summary: 'Update the global affiliate commission', security, request: { headers: csrf, body: json(z.object({ expectedVersion: z.number().int().min(1), commissionBps: z.number().int().min(0).max(10000) })) }, responses: ok('Affiliate settings updated', z.unknown()) });
