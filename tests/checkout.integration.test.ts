...
describe('checkout invariants', () => {
  ...
  beforeAll(async () => {
    ...
    const registered = await request(app).post('/api/v1/auth/register').send({ email, password: 'correct horse battery staple', name: 'Buyer' });
    expect(registered.status).toBe(201);
    await prisma.user.update({ where: { email }, data: { emailVerifiedAt: new Date() } });
    userAgent = request.agent(app);
    const login = await userAgent.post('/api/v1/auth/login').send({ email, password: 'correct horse battery staple' });
  ...
  it('revalidates stock and returns the same order for an idempotent retry', async () => {
    ...
    const first = await userAgent.post('/api/v1/orders').set('X-CSRF-Token', csrfToken).set('Idempotency-Key', idempotencyKey).send(body);
    expect(first.status).toBe(201);
    expect(first.body.order.payment.method).toBe('BANK_TRANSFER');
    const second = await userAgent.post('/api/v1/orders').set('X-CSRF-Token', csrfToken).set('Idempotency-Key', idempotencyKey).send(body);
